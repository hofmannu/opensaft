...
PlotStatus Plot(const char* label, const PlotConfig& conf) {
    ...
    const ImGuiID id = window->GetID(label);
		...
		ImVec2 frame_bb2;
		frame_bb2.x = window->DC.CursorPos.x + conf.frame_size.x;
		frame_bb2.y = window->DC.CursorPos.y + conf.frame_size.y;
    const ImRect frame_bb(window->DC.CursorPos, frame_bb2);

		ImVec2 innerbb1;
		innerbb1.x = frame_bb.Min.x + style.FramePadding.x;
		innerbb1.y = frame_bb.Min.y + style.FramePadding.y;
		// frame_bb.Min + style.FramePadding
		
		ImVec2 innerbb2;
		innerbb2.x = frame_bb.Max.x - style.FramePadding.x;
		innerbb2.y = frame_bb.Max.y - style.FramePadding.y;
	  //frame_bb.Max - style.FramePadding
		
    const ImRect inner_bb(innerbb1, innerbb2);
