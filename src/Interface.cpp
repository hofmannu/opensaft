...
void Interface::DataLoaderWindow() {
  ...
  if (isDataSetDefined) {
    if (ImGui::CollapsingHeader("Data information")) {
      ...
      ImGui::Text("%i x %i x %i", inputDataVol->get_dim(0),
    ...
    if (ImGui::CollapsingHeader("Raw Data slicer")) {
      ...
      ImGui::SliderFloat("MinVal", inDataMapper.get_pminVal(),
                         inputDataVol->get_minVal(), inputDataVol->get_maxVal(),
                         "%.1f");
    ...
    if (ImGui::CollapsingHeader("MIP preview")) {
      ...
      if (inputDataVol->get_updatedCropRange()) inputDataVol->calcCroppedMips();
...
void Interface::ReconWindow() {
  ...
  if (isReconDone)  // this means that the reconstruction was performed at least
                    // once
  {
    ...
    if (ImGui::CollapsingHeader("Slice preview recon")) {
      ...
      ImGui::SliderFloat("Min val recon slice", reconDataMapper.get_pminVal(),
                         reconDataVol->get_minVal(), reconDataVol->get_maxVal(),
                         "%.1f");
    ...
    if (ImGui::CollapsingHeader("MIP preview recon")) {
      ...
      // check if all are in a good valid order, otherwise make sure that they
      // are sorted
      ...
      // update cropped mips if something changed
      if (reconDataVol->get_updatedCropRange()) reconDataVol->calcCroppedMips();
    ...
    // this is not implemented yet but at some point we want to have an option
