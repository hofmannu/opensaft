	...
	for (uint32_t yIm = 0; yIm < nY; yIm++)
	{
		for (uint32_t xIm = 0; xIm < nX; xIm++)
		{
			for (uint32_t zIm = 0; zIm < nT; zIm++)
			{
				...
				if (rfsaftabs > 0)
				{
					const float cf = sett.get_flagCoherenceW() ? rfsaft * rfsaft / 
...
void saft::recon()
{
	  ...
	  *h_progress = 0;
    	...
    	if (value1 > value)
    	{
      	// printf("h_progress = %d\n", value1);
       	...
       	const double tPassed = std::chrono::duration_cast<std::chrono::seconds>(tCurr - tStart).count();
	...
	reconTime = std::chrono::duration_cast<std::chrono::seconds>(tEnd - tStart).count();
